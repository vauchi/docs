<!-- SPDX-FileCopyrightText: 2026 Mattia Egloff <mattia.egloff@pm.me> -->
<!-- SPDX-License-Identifier: GPL-3.0-or-later -->

# Vauchi Documentation

Privacy-focused updatable contact cards via
in-person exchange. End-to-end encrypted,
decentralized.

---

## What is Vauchi?

Vauchi is a contact card that updates
automatically. When you change your phone number,
everyone who has your card sees the change.

- **No sign-up required** — Your device is your
  identity
- **No phone number required** — Exchange contact
  cards in person
- **End-to-end encrypted** — Only you and your
  contacts can read your data
- **Open source** — Verify every claim yourself

## Quick Links

| | |
|---|---|
| 👤 **[Getting Started](users/getting-started.md)** | Set up and exchange |
| ❓ **[FAQ](users/faq.md)** | Common questions |
| 🔒 **[Security](about/security.md)** | Data protection |
| 💜 **[Our Principles](about/principles.md)** | Why we built it |

## For Developers

- [Contributing Guide](developers/contributing.md)
  — How to contribute to Vauchi
- [Architecture Overview](developers/architecture.md)
  — System design and components
- [Cryptography Reference](developers/crypto.md)
  — Encryption and key management details

## Get the App

- **Desktop**: Coming soon
  (macOS, Windows, Linux)
- **CLI/TUI**: Coming soon
- **iOS**: Coming soon
- **Android**: Coming soon

## Source Code

[GitLab](https://gitlab.com/vauchi) ·
[GitHub Mirror](https://github.com/vauchi)

---

*Your privacy matters. Vauchi is built to prove it.*
