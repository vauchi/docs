...
```json
{
  "contact_card": { /* user's contact card */ },
  "contacts": [
    { "id": "...", "public_key": "...", "card": { /* contact's card */ } }
  ],
  "visibility_rules": { /* per-contact visibility */ },
...
sequenceDiagram
    ...
    participant DB as Device B (Desktop) 🖥️
...
sequenceDiagram
    participant U as User
    participant DA as Device A 📱
    participant DB as Device 11 📱

    Note over U: Already has 10 linked devices

    U->>DA: Try to link 11th device
    DA->>DA: Check device count: 10
    DA->>U: "Maximum devices reached (10)"
    DA->>U: "Please unlink a device first"
```
...
- [Contact Exchange](01-contact-exchange.md) - Similar proximity verification
- [Sync Updates](03-sync-updates.md) - How changes sync between linked devices
- [Contact Recovery](04-contact-recovery.md) - Recovery when all devices lost
